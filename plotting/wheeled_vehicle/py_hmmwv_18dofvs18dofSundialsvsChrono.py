...
if (len(sys.argv) != 3):
    print("usage: python3 hmwmv_18dofvs18dofSundialsvsChrono.py <file> <save>")
    exit()
...
# 8dof file name
dataSun = pd.read_csv(
    path_out + file + "_hmmwv18SundialsPy.csv", sep=' ', header=None)

data8 = pd.read_csv(
    path_out + file + "_hmmwv18HiPy.csv", sep=' ', header='infer')
...
if (save):
    fig.savefig(f'./images/hmmwv_chvs18_{file}.eps', format='eps', dpi=3000)
mpl.show()
