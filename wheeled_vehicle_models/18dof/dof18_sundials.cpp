...
UserData::UserData(int param_flags,
    ...
    int offset = veh_param._tcbool ? 1 : 0;
    m_vx_idx = 4 + offset;  // Since there are no deflection states in the tires
    m_vy_idx = 5 + offset;  // Since there are no deflection states in the tires
    m_tire_type = TireType::TMeasyNr;
}
...
const d18::TMeasyParam& UserData::GetTireTMParam() {
    return m_tireTM_param;
}

const d18::TMeasyNrParam& UserData::GetTireTMNrParam() {
...
void d18SolverSundials::Construct(const std::string& vehicle_params_file,
    ...
    if (m_tire_type == TireType::TMeasy) {
        ...
        m_data.m_vx_idx = 4 + m_offset;
        m_data.m_vy_idx = 5 + m_offset;
    }
...
void d18SolverSundials::Write(CSV_writer& csv, realtype t, N_Vector y, N_Vector* yS) {
    ...
    if (m_tire_type == TireType::TMeasy) {
        ...
        csv << Ith(y, 4 + m_offset) << Ith(y, 5 + m_offset) << Ith(y, 6 + m_offset) << Ith(y, 7 + m_offset)
            << Ith(y, 8 + m_offset) << Ith(y, 9 + m_offset);
    }
    ...
    if (yS) {
        for (int is = 0; is < m_ns; is++)
            if (m_tire_type == TireType::TMeasy) {
                ...
                csv << Ith(yS[is], 4 + m_offset) << Ith(yS[is], 5 + m_offset);
            }
    }
    csv << std::endl;
}
...
int d18::rhsFun(realtype t, N_Vector y, N_Vector ydot, void* user_data) {
...
int d18::rhsQuad(realtype t, N_Vector y, N_Vector qdot, void* user_data) {
    // Unpack user data
    UserData* udata = (UserData*)user_data;
...
int d18::printStatsCvode(void* cvode_mem) {
    int retval = 0;
...
void d18::printSUNMatrix(SUNMatrix A, sunindextype matrows, sunindextype matcols) {
