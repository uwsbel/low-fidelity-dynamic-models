...
__host__ void d24GPU::initializeTireSus(const VehicleState* v_states,
    ...
    double weight_split =
        ((v_params->_m * G * v_params->_b) / (2 * (v_params->_a + v_params->_b)) + v_params->_muf * G);
...
__device__ void d24GPU::vehToSusTransform(const VehicleState* v_states,
    ...
    suslf_st->_ws =
        (v_params->_cf * v_states->_wx / 2.) - (v_params->_a * v_states->_wy) + v_states->_w;  // z direction
...
__device__ void d24GPU::computeVehicleRHS(VehicleState* v_states,
    ...
    v_states->_wydot = (1 / v_params->_jy) * ((suslf_st->_my + susrf_st->_my + suslr_st->_my + susrr_st->_my) +
                                              (suslr_st->_fzs + susrr_st->_fzs) * v_params->_b -
                                              (suslf_st->_fzs + susrf_st->_fzs) * v_params->_a);

    v_states->_wzdot = (1 / v_params->_jz) * ((suslf_st->_mz + susrf_st->_mz + suslr_st->_mz + susrr_st->_mz) +
                                              (suslf_st->_fys + susrf_st->_fys) * v_params->_a -
                                              (suslr_st->_fys + susrr_st->_fys) * v_params->_b +
