...
struct TMeasyState {
    ...
    /// equilibrium. This is also largely untested and its thus recommended to initialize the states to 0 or use the copy
    /// constructor below
...
struct TMeasyNrState {
    ...
    // States that are integrated - these are actually wheel states but we don't use that abstraction
    double _omega;   //!< Angular velocity of wheel
    double _dOmega;  //!< Angular acceleration of wheel
    double _xt;      //!< Vertical tire compression
    double _dxt;     //!< Vertical tire compression velocity
...
/// more details https://uwmadison.box.com/s/2tsvr4adbrzklle30z0twpu2nlzvlayc.
struct VehicleParam {
    ...
    std::vector<double> _gearRatios;  //!< A vector of gear ratios
     ...
     */
    std::vector<MapEntry> _CFmap;

    /**
     * @brief In case a torque converter is part of the model, the torque ratio map needs to be defined.
     *
     * _TRmap is a vector of MapEntry's where the x component of the MapEntry is the speed ratio between the
     * driven and driving shafts (whether the engine crank is driven or driving depends on if there is a throttle
...
///  https://uwmadison.box.com/s/2tsvr4adbrzklle30z0twpu2nlzvlayc.
struct VehicleState {
    /// @brief Default constructor that sets all the states to zero
    VehicleState()
        : _x(0.),
...
struct SuspensionState {
    ...
    SuspensionState(const SuspensionState& other)
        : _xs(other._xs),
          _xsi(other._xsi),
          _dxs(other._dxs),
          _ls(other._ls),
